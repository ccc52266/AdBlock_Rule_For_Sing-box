...
# DNS规范验证函数
function Is-ValidDNSDomain($domain_suffix) {
    if ($domain_suffix.Length -gt 253) { return $false }
    $labels = $domain_suffix -split "\."
...
foreach ($url in $urlList) {
    ...
    try {
        ...
        foreach ($line in $lines) {
            ...
            if ($line.StartsWith('@@')) {
                $domain_suffixs = $line -replace '^@@', '' -split '[^\w.-]+'
                foreach ($domain_suffix in $domain_suffixs) {
                    if (-not [string]::IsNullOrWhiteSpace($domain_suffix) -and $domain_suffix -match '[\w-]+(\.[[\w-]+)+') {
                        $excludedDomains.Add($domain_suffix.Trim()) | Out-Null
                    }
                }
            }
            else {
                ...
                if ($line -match '^\|\|([a-zA-Z0-9.-]+\.[a-zA-Z]{2,})\^$') {
                    $domain_suffix = $Matches[1]
                    $uniqueRules.Add($domain_suffix) | Out-Null
                }
                # 匹配 Hosts 文件格式的 IPv4 规则
                elseif ($line -match '^(0\.0\.0\.0|127\.0\.0\.1)\s+([a-zA-Z0-9.-]+\.[a-zA-Z]{2,})$') {
                    $domain_suffix = $Matches[2]
                    $uniqueRules.Add($domain_suffix) | Out-Null
                }
...
foreach ($domain_suffix in $uniqueRules) {
    if (Is-ValidDNSDomain($domain_suffix)) {
        $validRules.Add($domain_suffix) | Out-Null
    }
}

foreach ($domain_suffix in $excludedDomains) {
    if (Is-ValidDNSDomain($domain_suffix)) {
        $validExcludedDomains.Add($domain_suffix) | Out-Null
    }
}
...
Add-Content -Path $logFilePath -Value "Total entries: $ruleCount""https://raw.githubusercontent.com/AdguardTeam/FiltersRegistry/master/filters/ThirdParty/filter_229_IdontCareAboutCookies/filter.txt",  
"https://raw.githubusercontent.com/AdguardTeam/FiltersRegistry/master/filters/ThirdParty/filter_239_FanboyAntifonts/filter.txt",  
"https://raw.githubusercontent.com/AdguardTeam/FiltersRegistry/master/filters/ThirdParty/filter_240_BarbBlock/filter.txt",  
"https://raw.githubusercontent.com/AdguardTeam/FiltersRegistry/master/filters/ThirdParty/filter_241_FanboyCookiemonster/filter.txt",  
"https://raw.githubusercontent.com/AdguardTeam/FiltersRegistry/master/filters/ThirdParty/filter_242_NoCoin/filter.txt",  
